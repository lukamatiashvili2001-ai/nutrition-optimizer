...
def run_optimizer(target_calories, target_protein, target_carbs,
    ...
    if previous_items:
        for pi in previous_items:
            # მხოლოდ ის პროდუქტები რომლებსაც remaining_grams > 0
    ...
    # წინა კალათის პროდუქტები ახლიდან არ შემოგვთავაზოს
    ...
    # ჯერ წინა კალათის დარჩენილი პროდუქტები
    if previous_items:
        for pi in previous_items:
            if pi.get('remaining_grams', 0) <= 0:
                continue
...
def save_basket():
    ...
    replace_id    = data.get('replace_basket_id')  # ძველი კალათის ID წასაშლელად

    # თუ replace_id გადმოვიდა — ძველი კალათა წავშალოთ
    if replace_id:
