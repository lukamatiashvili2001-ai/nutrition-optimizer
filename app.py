...
# ── CSV ერთხელ ჩაიტვირთება გაშვებისას, მეხსიერებაში რჩება ──────────────────
_products_df = None
...
def get_products():
    ...
    if _products_df is None:
        ...
        df = df[df['kcal_per_package'] > 0].reset_index(drop=True)
        _products_df = df
    return _products_df
...
def run_optimizer(target_calories, target_protein, target_carbs,
    ...
    # წინა კალათის დარჩენილი მაკროები
    ...
    # ობიექტური ფუნქცია: ფასების მინიმიზაცია (მთლიანი შეფუთვების ფასი)
    ...
    if calorie_mode == 'cap':
        ...
        b_lo.append(eff_cal * 0.95)
        b_hi.append(eff_cal)
    else:
        A_rows.append(df['kcal_per_package'].values.astype(float))
        b_lo.append(eff_cal * 0.95)
        b_hi.append(eff_cal * 1.05)
    ...
    # MILP — integer constraints (მთლიანი შეფუთვები)
    constraints = LinearConstraint(A_matrix,
                                   lb=np.array(b_lo),
                                   ub=np.array(b_hi))
    bounds = Bounds(lb=np.zeros(n), ub=np.full(n, float(max_packages)))

    # integrality=1 ნიშნავს integer ცვლადი
    result = milp(c,
    ...
    if not result.success:
        # fallback: continuous relaxation თუ MILP ვერ მოხერხდა
        A_ub, b_ub = [], []
        if calorie_mode == 'cap':
        ...
        fb = [(0, float(max_packages)) for _ in range(n)]
        result2 = linprog(c, A_ub=np.array(A_ub), b_ub=np.array(b_ub), bounds=fb, method='highs')
        if result2.status != 0:
            return None, 'ოპტიმიზაცია ვერ მოხერხდა. სცადე კალორიების გაზრდა ან მაქს. შეფუთვების გაზრდა.'
        x = np.round(result2.x).astype(int)
    else:
    ...
    # წინა კალათის დარჩენილი პროდუქტები
    if previous_items:
        for pi in previous_items:
            if pi.get('remaining_grams', 0) <= 0:
                continue
            ...
            factor = rem / 100.0
            cal  = round(pi.get('cal_per_100',  0) * factor, 1)
            prot = round(pi.get('prot_per_100', 0) * factor, 1)
            carb = round(pi.get('carb_per_100', 0) * factor, 1)
            fat  = round(pi.get('fat_per_100',  0) * factor, 1)
            basket.append({
                'name': pi['product_name'], 'units': 0,
                'pkg_label': 'წინა კალათა', 'is_previous': True,
                'recommended_grams': round(rem, 1), 'remaining_grams': 0,
                'cost': 0, 'calories': cal, 'protein': prot, 'carbs': carb, 'fat': fat,
    ...
    # ახალი პროდუქტები — მთლიანი შეფუთვები
    for i, row in df.iterrows():
        ...
        # მაქს 200გ/დღე შეზღუდვა — რამდენს მიირთმევს
        rec_grams = min(units * pkg_size, 200 * units)
        ...
        # სალაროზე გადასახდელი — მთლიანი შეფუთვების ფასი (integer!)
...
def save_basket():
    ...
    if replace_id:
        old = Basket.query.filter_by(id=replace_id, user_id=current_user.id).first()
        if old:
